...
# 📦 Extract data
# ==============================================================================
DATA_TARBALL="/staging/hhao9/temposub_data_lowdim_5subtypes.tar.gz"

if [[ -f "$DATA_TARBALL" ]]; then
    echo "📦 Extracting $DATA_TARBALL..."
    tar -xzf "$DATA_TARBALL"
    # If extraction creates "temposub_data", rename to "data"
    if [[ -d "temposub_data_lowdim_5subtype" ]]; then
        rm -rf data   # remove old data folder if it exists
        mv temposub_data_lowdim_5subtype data
        echo "Renamed temposub_data_lowdim_5subtype -> data"
    fi
else
